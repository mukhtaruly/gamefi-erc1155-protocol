...
contract DeployProtocol is Script {
    function run() external {
        ...
        address vrfCoordinator = vm.envOr("VRF_COORDINATOR", address(0x0000000000000000000000000000000000000001));
        ...
        GameTreasury treasury = new GameTreasury(address(timelock));
        RentalVault vault = new RentalVault(token, items);
        new ProtocolFactory(deployer);
        VRFLootChest lootChest =
            new VRFLootChest(items, IVRFCoordinatorV2Plus(vrfCoordinator), keyHash, subscriptionId, 3, 200_000);

        items.grantRole(items.MINTER_ROLE(), address(lootChest));
